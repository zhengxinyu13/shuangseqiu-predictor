...
5. **排除与历史完全相同的红球组合**（与整本历史全库比对）。
   这条**实测几乎没有作用，如实记在这里**：历史只占红球组合空间的两千分之一量级
   （全量期数 ÷ C(33,6) ≈ 0.32%），单次选号撞上的概率不到千分之三；
   而且历史里那几对红球重复**基本都落在本策略的候选空间之外**
   （和值 <120 或没有 32/33），所以它极少真正拦下输出。
...
class SelectionStrategy:
    ...
    def __init__(
        ...
        # 历史里会有红球完全相同的期，所以「期数」与「去重组数」是两个数：
        # 说明文案要同时给出，免得界面上一处写期数、一处写组数自相矛盾。
